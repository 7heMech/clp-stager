...
Un script de Bash ligero e interactivo para CloudPanel que clona un sitio web de producción en un entorno de staging totalmente funcional en cuestión de segundos.

Realiza automáticamente lo siguiente:

1. Aprovisiona un nuevo sitio de staging utilizando la misma versión de PHP que la producción.

2. Clona los archivos mediante un pipeline de `tar` rápido para preservar la integridad.

3. Detecta tu base de datos de producción, la exporta y crea una nueva base de datos para el staging.

4. Importa los datos en la base de datos de staging.

5. Actualiza automáticamente Laravel (`.env`) o WordPress (`wp-config.php`) con las nuevas credenciales de la base de datos de staging.

6. Clona la configuración de Varnish.

7. Solicita la emisión de certificados de Let's Encrypt listando todos los dominios requeridos; asegúrate de haber configurado los registros DNS antes de proceder.

## 🚀 Cómo Instalar y Ejecutar en tu Servidor

No es necesario clonar todo este repositorio en tu servidor. Puedes crear y ejecutar el script directamente vía SSH.

### Paso 1: Conéctate a tu servidor

Accede por SSH a tu servidor CloudPanel como usuario `root`:

```bash
ssh root@your-server-ip
```

### Paso 2: Crea el archivo del script

Abre el editor de texto `nano` para crear el archivo:

```bash
nano /root/make-staging.sh && chmod +x /root/make-staging.sh
```

Copia el código de abajo, pégalo en el editor `nano`, guarda (CTRL+S) y sal (CTRL+X):


<details markdown="1">
<summary><strong>Haz clic para expandir el Script de Bash</strong></summary>
...
Siempre que quieras levantar un sitio de staging, simplemente ejecuta:

```bash
/root/make-staging.sh
```

Usa las teclas de flecha para seleccionar el sitio de producción, escribe tu dominio de staging (ej. `stg.example.com`) y el script clonará automáticamente los archivos y la base de datos.
*(Nota: Asegúrate de haber apuntado el Registro A de DNS de tu dominio de staging a la IP de tu servidor).*

## 🧹 Limpieza (Eliminar el Sitio de Staging)

Una vez que hayas terminado de probar tus cambios y los hayas subido a producción, puedes limpiar fácilmente el entorno de staging para liberar recursos del servidor.

Puedes eliminar el sitio y la base de datos directamente desde la **interfaz gráfica de CloudPanel**, o si prefieres la línea de comandos, simplemente ejecuta los siguientes dos comandos (reemplaza `stg.example.com` y `database_name` con los valores generados por el script):

```bash
# 1. Eliminar la base de datos
clpctl db:delete --databaseName=your_staging_database_name

# 2. Eliminar el sitio de staging y los archivos del usuario
